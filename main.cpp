 ...
 */
static bool write_output(const std::string& json_str, const std::string& output_file) {
...
static int run_single_pid(const memc::CLIOptions& opts) {
    ...
    if (opts.count == 1) {
        ...
        std::cerr << "Sampling PID " << opts.pid << " every " << opts.collector_config.interval_ms
                  << "ms" << (opts.collector_config.use_smaps ? " (with smaps)" : "")
